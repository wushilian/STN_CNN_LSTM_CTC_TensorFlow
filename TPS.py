...
def ThinPlateSpline2(U, source, target, out_size):
  ...
  """
    ...
    def _interpolate(im, x, y, out_size):
        ...
        max_y = tf.cast(im_shape[1] - 1, 'int32')
        max_x = tf.cast(im_shape[2] - 1, 'int32')
    ...
    def _meshgrid(height, width, source):
        ...
        num_batch = source.get_shape().as_list()[0]
    ...
    def _transform(T, source, input_dim, out_size):
        inputdim_shape=input_dim.get_shape().as_list()
        num_batch = inputdim_shape[0]
        height = inputdim_shape[1]
        width = inputdim_shape[2]
        num_channels = inputdim_shape[3]
    ...
    def _solve_system(source, target):
        num_batch = source.get_shape().as_list()[0]
        num_point = source.get_shape().as_list()[1]
